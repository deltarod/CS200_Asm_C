...
{ 
 // TODO: compress the data instead of just writing it out to the file
 	//counts number of times the character appears 
	int repeat;
	//character appears atleast once
	repeat = 1;
	//character until evaluation
	char ch;
	ch = 0;
 ...
 { 
 	...
 	if(i < (count - 1)){
 		//while the next element is the same as the current
 		while (data[i+1] == data[i]){
			...
			//while loop exists when there is a new character
 			/*using the example from the project guide of 
 			For example, if you figure out that the next 5 bytes are the same, you 
	 		could say "putc(5,outfile); putc(data[i],outfile);*/
	 	
	 		if (ch = 0){
	 			//no repeats
	 			putc(repeat,outfile); putc(data[i], outfile ); // write out a single byte of data	
	 		}else{
	 			//repeats
	 			putc(repeat,outfile); putc(ch, outfile ); // write out a single byte of data
	 		}
			//refresh the counter
			repeat = 0;
		}
...
{  // TODO: decompress the data instead of just writing it out to the file 
 ...
 { 
 	...
 	if(isdigit(data[i])){
 		//print the character that many times
 		for(int j=0; j<i; j++){
 			//print the element before the digit, which is the repeating character
 			putc( data[i-1], outfile ); // write out a single byte of data 
 		}
